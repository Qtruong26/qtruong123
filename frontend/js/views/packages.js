VIEWS.packages = async function () {
  ...
  <div class="topbar">
    <div>
      ...
      <div class="page-title">Gói tập & Thanh toán</div>
      <div class="page-desc">Quản lý các gói tập và đăng ký thành viên.</div>
    </div>

    ${
      SESSION.role === 'admin'
        ? `
          <button class="btn btn-secondary" onclick="openPackageModal()">
            + Gói mới
          </button>
        `
        : ''
  ...
  <div class="panel packages-panel">
    <div class="panel-head">
      <div>
        <div class="panel-title">CÁC GÓI TẬP</div>
        <div class="hint">${packages.length} gói đang hoạt động</div>
      </div>
    </div>
  ...
  <div class="panel">
    <div class="panel-head">
      <div class="panel-title">Đăng ký / Gia hạn gói</div>
      ...
      <table>
        ...
        <tbody>
          ${
            ...
            <tr>
              <td colspan="5">
                <div class="empty-state">Chưa có đăng ký nào.</div>
              </td>
            </tr>
            `
          }
  ...
  <div class="panel">
    <div class="panel-head">
      <div class="panel-title">Lịch sử thanh toán</div>
    </div>
...
function openPackageModal() {
  openModal(`
    ...
    <div class="field">
      <label>Tên gói</label>
      <input id="pf_name" placeholder="Ví dụ: Gym Basic">
    </div>

    <div class="form-row">
      <div class="field">
        <label>Thời hạn (ngày)</label>
        <input id="pf_days" type="number" value="30">
      </div>

      <div class="field">
        <label>Giá (VNĐ)</label>
        <input id="pf_price" type="number" value="600000">
      </div>
    </div>

    <div class="field">
      <label>Link hình ảnh</label>
      <input id="pf_image" type="url" placeholder="https://...">
      <div class="hint">Có thể để trống, hệ thống sẽ tự chọn ảnh.</div>
    </div>

    <div class="field">
      <label>Mô tả</label>
      <textarea id="pf_desc" rows="3" placeholder="Mô tả gói tập..."></textarea>
    </div>
...
async function savePackage() {
  ...
  const durationDays = Number(document.getElementById('pf_days').value);
  const price = Number(document.getElementById('pf_price').value);
  const description = document.getElementById('pf_desc').value.trim();
  const imageUrl = document.getElementById('pf_image').value.trim();

  if (!name) {
    toast('Vui lòng nhập tên gói.', true);
    return;
  }
  ...
  openModal(`
    ...
    <div class="field">
      ...
      <select id="ef_package">
        ${packages.map(p => `<option value="${p.id}">${p.name} — ${fmtMoney(p.price)}</option>`).join('')}
      </select>
    </div>
    ...
    <div class="form-row">
      <div class="field">
        ...
        <input id="ef_start" type="date" value="${todayStr()}">
      </div>
...
/* =========================================================
   SUBMIT ĐĂNG KÝ (ĐÃ FIX: thêm catch và đóng ngoặc)
========================================================= */
async function submitEnroll() {
  ...
  try {
    const data = await Api.post('/packages/enrollments', {
      memberId,
      packageId,
      startDate,
      method
    ...
    if (data.requiresPayment) {
      ...
      toast(data.message || 'Đã đăng ký gói và ghi nhận thanh toán.');
      closeModal();
  ...
  try {
    ...
    if (result.requiresPayment) {
      ...
      openModal(`
        ...
        <div style="text-align:center; background:#fff; border-radius:14px; padding:18px; margin-bottom:16px;">
          <img
            src="${result.qrUrl}"
            alt="QR thanh toán"
            style="width:280px; max-width:100%; height:auto; display:block; margin:0 auto;"
          >
        </div>
        ...
        <div class="panel" style="margin-bottom:14px;">
          <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
            ...
            <b style="color:var(--volt); font-size:18px;">${fmtMoney(result.amount)}</b>
          </div>
        ...
        <div class="hint" style="padding:12px; border-radius:10px; background:rgba(255,255,255,.04); margin-bottom:16px;">
          Sau khi chuyển khoản thành công, hãy bấm <b>"Tôi đã thanh toán"</b> để xác nhận đăng ký.
        </div>

        <div class="modal-actions">
          <button class="btn btn-secondary" onclick="closeModal()">Hủy</button>
          <button class="btn btn-primary" onclick='confirmSelfPayment(${JSON.stringify(pending)})'>
            Tôi đã thanh toán
          </button>
        </div>
      `);
...
async function confirmSelfPayment(pendingEnrollment) {
  try {
    ...
    closeModal();
    toast(result.message || 'Đã xác nhận thanh toán và đăng ký gói thành công.');
    navigate('myPackages');
  } catch (err) {
...
/* =========================================================
   XÁC NHẬN THANH TOÁN (ĐÃ FIX: thêm dấu đóng hàm })
========================================================= */
async function confirmPaymentDone(memberId, packageId, startDate, method) {
  try {
    await Api.post('/packages/enrollments/confirm-payment', {
      memberId,
      packageId,
      startDate,
      method
...
/* =========================================================
   GẮN VÀO WINDOW ĐỂ HTML ONCLICK GỌI ĐƯỢC
========================================================= */
window.selfEnroll = selfEnroll;
window.confirmSelfPayment = confirmSelfPayment;
window.confirmPaymentDone = confirmPaymentDone;
if (typeof openSelfEnrollModal !== 'undefined') {

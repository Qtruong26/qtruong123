VIEWS.members = async function () {
  ...
  const rows = members.map((m) => `
    <tr>
      <td>${m.name}</td><td class="mono">${m.phone || ''}</td><td>${m.goal || '—'}</td>
      <td>${m.joinDate}</td><td>${m.activePackage ? m.activePackage.packageName : '—'}</td>
  ...
  return `
  <div class="topbar">
    <div><div class="page-eyebrow">Vận hành</div><div class="page-title">Hội viên</div>
      <div class="page-desc">Quản lý hồ sơ hội viên và trạng thái gói tập hiện tại.</div></div>
  ...
  <div class="panel">
    ...
    <div class="table-wrap"><table id="memberTable">
      <thead><tr><th>Họ tên</th><th>SĐT</th><th>Mục tiêu</th><th>Ngày tham gia</th><th>Gói hiện tại</th><th>Trạng thái</th><th></th></tr></thead>
...
async function saveMember(id) {
  ...
  const data = {
    name, phone: document.getElementById('f_phone').value.trim(), email: document.getElementById('f_email').value.trim(),
    goal: document.getElementById('f_goal').value, level: document.getElementById('f_level').value,
  };
  try {
...
function deleteMember(id) {
  confirmAction('Xóa hội viên này? Toàn bộ lịch sử liên quan sẽ được giữ nguyên nhưng không hiển thị hồ sơ.', async () => {
    try { await Api.del(`/members/${id}`); toast('Đã xóa hội viên.'); navigate('members'); } catch (err) { showApiError(err); }
  });
}
